...
(def neighborhood [(v2 -1.0 0.) (v2 -1. 1.) (v2 -1. -1.) (v2 1. 0.) (v2 1. 1.) (v2 1. -1.) (v2 0. 1.) (v2 0. -1.)])
...
(def dead (v4 0. 0. 0. 1.))
(deftype GLSLEnv {time float resolution vec2})
(defn drawToBuffer [env:GLSLEnv fragCoord:vec2 buffer:texture] 1.0 
 (if (< env.time 0.01) 
 (if (> (random (/ fragCoord env.resolution)) 0.95) live dead) 
 (let [self (isLive ([(/ fragCoord env.resolution 1.0)] buffer)) neighbors (countNeighbors fragCoord env.resolution buffer)] 
 (if (|| (&& self (== 2 neighbors)) (== neighbors 3)) live dead))) 1.0)