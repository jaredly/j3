...
let state_f = ({"0": f}) => f
let state$slnil = $co(0)($co($co(nil)($co(nil)(nil)))(map$slnil))
...
let applied_types = (types) => (subst) => map(types)(({"1": {"1": keep, "0": type}, "0": loc}) => (($target) => {
if ($target === true) {
return $co(loc)(type)
} ;
...
let unwrap_type_tuple = (arg2) => loop(arg2)((arg) => (recur) => (($target) => {
if ($target.type === "tapp") {
if ($target[0].type === "tapp") {
if ($target[0][0].type === "tcon") {
if ($target[0][0][0] === ",") {
...
return cons(arg)(nil);
throw new Error('match fail 24680:' + JSON.stringify($target))
})(arg))
...
throw new Error('match fail 25289:' + JSON.stringify($target))
})(type)
...
throw new Error('match fail 25391:' + JSON.stringify($target))
})($eq(0)(arity)))
...
return f(init)(pat);
throw new Error('match fail 25561:' + JSON.stringify($target))
})(pat)
...
return some(id);
throw new Error('match fail 25636:' + JSON.stringify($target))
}
} ;
throw new Error('match fail 25625:' + JSON.stringify($target))
}
} ;
return gid;
throw new Error('match fail 25617:' + JSON.stringify($target))
})(pat))
let concat$ti = (lists) => (($target) => {
if ($target.type === "nil") {
return nil
} ;
if ($target.type === "cons") {
...
throw new Error('match fail 26026:' + JSON.stringify($target))
})(lists)
let length = (lst) => (($target) => {
if ($target.type === "nil") {
return 0
} ;
if ($target.type === "cons") {
...
throw new Error('match fail 26071:' + JSON.stringify($target))
})(lst)
...
let l = $target[2];
{
let args = cons(arg1)(unwrap_type_tuple(arg2));
{
let free$0 = free;
{
let args$1 = args;
{
let {"1": free, "0": args} = tts_list(args$1)(free$0)(locs);
return $co(and_loc(locs)(l)(`(, ${join(" ")(rev(args)(nil))})`))(free)
}
}
}
}
...
{
let name = at(letters)(idx)("_too_many_vbls_");
return $co(cst$slid(name)(loc))($co(some(map$slset(fmap)(vname)(name)))(1 + idx))
}
};
...
return $co(cst$sllist(cons(cst$slid("fn")(la))(cons(cst$slarray(rev(args)(nil))(la))(cons(two)(nil))))(l))(free)
}
}
}
}
...
{
let {"1": free, "0": args} = ttc_list(args$1)(free$0);
return $co(cst$sllist(cons(cst$slid(",")(l$co))(rev(args)(nil)))(l))(free)
}
}
}
}
...
return nil;
throw new Error('match fail 25520:' + JSON.stringify($target))
}
} 
} ;
if ($target.type === "cons") {
...
throw new Error('match fail 25496:' + JSON.stringify($target))
})(row)
...
let tenv$slset_constructors = ({"3": alias, "2": names, "1": cons, "0": types}) => (name) => (vbls) => (ncons) => (loc) => tenv(types)(map$slmerge(cons)(ncons))(map$slset(names)(name)($co(vbls)($co(set$slfrom_list(map$slkeys(ncons)))(loc))))(alias)
...
let tenv$sladd_alias = ({"3": aliases, "2": c, "1": b, "0": a}) => (name) => ({"1": {"1": l, "0": body}, "0": args}) => tenv(a)(b)(c)(map$slset(aliases)(name)($co(args)($co(body)(l))))
let tenv$sladd_builtin_type = ({"3": d, "2": names, "1": b, "0": a}) => ({"1": args, "0": name}) => tenv(a)(b)(map$slset(names)(name)($co(args)($co(set$slnil)(-1))))(d)
...
return $lt_err(tmissing(map(zip(missing)(map(missing_vars)(type_apply(subst))))(({"1": type, "0": {"1": loc, "0": name}}) => $co(name)($co(loc)(type)))))
}
};
throw new Error('match fail 15854:' + JSON.stringify($target))
})(missing_vars)
let find_missing_names = (values) => (ex) => {
let externals = bag$slfold((ext) => ({"1": {"1": l}, "0": name}) => map$slset(ext)(name)(l))(map$slnil)(ex);
...
let idx_$gt = (idx) => $gt$gt$eq($lt_state)(({"1": {"1": c, "0": b}}) => $gt$gt$eq(state_$gt($co(idx)($co(b)(c))))((_17445) => $lt_($unit)))
...
let record_$gt = (loc) => (type) => (keep) => $gt$gt$eq($lt_state)(({"1": {"1": subst, "0": {"1": usages, "0": types}}, "0": idx}) => $gt$gt$eq(state_$gt($co(idx)($co($co(cons($co(loc)($co(type)(keep)))(types))(usages))(subst))))((_17491) => $lt_($unit)))
let $lt_types = $gt$gt$eq($lt_state)(({"1": {"0": types}}) => $lt_(types))
let $lt_subst = $gt$gt$eq($lt_state)(({"1": {"1": subst}}) => $lt_(subst))
...
let record_usage_$gt = (loc) => (provider) => $gt$gt$eq($lt_state)(({"1": {"1": subst, "0": {"1": {"1": usages, "0": defs}, "0": types}}, "0": idx}) => $gt$gt$eq(state_$gt($co(idx)($co($co(types)($co(defs)(cons($co(loc)(provider))(usages))))(subst))))((_20663) => $lt_($unit)))
let record_def_$gt = (loc) => $gt$gt$eq($lt_state)(({"1": {"1": subst, "0": {"1": {"1": usages, "0": defs}, "0": types}}, "0": idx}) => $gt$gt$eq(state_$gt($co(idx)($co($co(types)($co(cons(loc)(defs))(usages)))(subst))))((_20720) => $lt_($unit)))
let subst_reset_$gt = (new_subst) => $gt$gt$eq($lt_state)(({"1": {"1": subst, "0": types}, "0": idx}) => $gt$gt$eq(state_$gt($co(idx)($co(types)(new_subst))))((_21254) => $lt_(subst)))
...
let tdefs_$gts = (tdefs) => (constructors) => map(map$slto_list(tdefs))(({"1": {"1": {"1": loc, "0": cnames}, "0": num_args}, "0": name}) => `\n - ${name}${join("")(map(set$slto_list(cnames))((cname) => `\n   - ${cname} ${(($target) => {
if ($target.type === "none") {
...
return "false";
throw new Error('match fail 25037:' + JSON.stringify($target))
})(v))("bool")(nil)
} 
} ;
if ($target.type === "pcon") {
...
let {"1": tcons} = tenv;
{
let {"2": cres, "1": cargs, "0": free_names} = (($target) => {
if ($target.type === "none") {
...
throw new Error('match fail 25077:' + JSON.stringify($target))
})(map$slget(tcons)(name));
{
let subst = map$slfrom_list(zip(free_names)(targs));
return ex$slconstructor(name)(tname)(map(zip(args)(map(cargs)(type_apply(subst))))(pattern_to_ex_pattern(tenv)))
}
}
}
}
} ;
throw new Error('match fail 24999:' + JSON.stringify($target))
})(pattern)
let default_matrix = (matrix) => concat$ti(map(matrix)((row) => (($target) => {
if ($target.type === "cons") {
if ($target[0].type === "ex/any") {
let rest = $target[1];
...
return nil;
throw new Error('match fail 25422:' + JSON.stringify($target))
})(row)))
...
throw new Error('match fail 25685:' + JSON.stringify($target))
}
};
throw new Error('match fail 25658:' + JSON.stringify($target))
})(gid)
let args_if_complete = (tenv) => (matrix) => {
let heads = map(matrix)((row) => (($target) => {
if ($target.type === "nil") {
...
throw new Error('match fail 25726:' + JSON.stringify($target))
})(row));
{
...
return found;
throw new Error('match fail 25768:' + JSON.stringify($target))
})(head)));
{
...
throw new Error('match fail 25813:' + JSON.stringify($target))
}
} ;
throw new Error('match fail 25804:' + JSON.stringify($target))
})(constrs))
};
throw new Error('match fail 25787:' + JSON.stringify($target))
}
}
} ;
...
throw new Error('match fail 25857:' + JSON.stringify($target))
}
} ;
throw new Error('match fail 25840:' + JSON.stringify($target))
})(matrix);
{
...
throw new Error('match fail 25919:' + JSON.stringify($target))
}
} ;
{
let alts = $target;
...
throw new Error('match fail 25910:' + JSON.stringify($target))
}
} ;
if ($target.type === "ex/or") {
let left = $target[0];
...
throw new Error('match fail 25962:' + JSON.stringify($target))
}
} ;
throw new Error('match fail 25885:' + JSON.stringify($target))
}
} 
} ;
throw new Error('match fail 25873:' + JSON.stringify($target))
}
}
...
let bag$slto_list = bag$slfold((list) => (a) => cons(a)(list))(nil)
...
let infer_deftype = (tenv$qu) => (mutual_rec) => (tname) => (tnl) => (targs) => (constructors) => (l) => $gt$gt$eq($lt_(map(constructors)(({"0": name}) => name)))((names) => $gt$gt$eq(record_def_$gt(tnl))((_14709) => $gt$gt$eq(foldl_$gt(tcon(tname)(tnl))(targs)((body) => ({"1": al, "0": arg}) => $gt$gt$eq(record_def_$gt(al))((_23329) => $lt_(tapp(body)(tvar(arg)(al))(l)))))((final) => $gt$gt$eq($lt_(map$slfrom_list(targs)))((free_map) => $gt$gt$eq($lt_(set$slfrom_list(map$slkeys(free_map))))((free_set) => $gt$gt$eq(foldl_$gt($co(map$slnil)(map$slnil))(constructors)(({"1": cons, "0": values}) => ({"1": {"1": {"1": l, "0": args}, "0": nl}, "0": name}) => $gt$gt$eq(map_$gt(type_with_free_rec(free_map))(args))((args) => $gt$gt$eq(do_$gt(record_usages_in_type(tenv$qu)(mutual_rec))(args))((_14791) => $gt$gt$eq(record_def_$gt(nl))((_14791) => $gt$gt$eq(map_$gt(subst_aliases(tenv$slalias(tenv$qu)))(args))((args) => $lt_($co(map$slset(values)(name)($co(scheme(set$slfrom_list(map$slkeys(free_map)))(foldr(final)(args)((body) => (arg) => tfn(arg)(body)(l))))(nl)))(map$slset(cons)(name)(tconstructor(map(targs)(fst))(args)(final)(nl))))))))))(({"1": cons, "0": values}) => $lt_(tenv(values)(cons)(map$slset(map$slnil)(tname)($co(len(targs))($co(set$slfrom_list(names))(tnl))))(map$slnil))))))))
...
let subst_$gt = (new_subst) => $gt$gt$eq($lt_state)(({"1": {"1": subst, "0": types}, "0": idx}) => $gt$gt$eq(state_$gt($co(idx)($co(types)(compose_subst("")(new_subst)(subst)))))((_17634) => $lt_($unit)))
...
return true;
throw new Error('match fail 25460:' + JSON.stringify($target))
...
return many(map(constructors)(({"1": {"1": {"0": args}, "0": l}, "0": name}) => (($target) => {
if ($target.type === "nil") {
return empty
} ;
...
let infer_stypes = (tenv$qu) => (stypes) => (salias) => $gt$gt$eq($lt_(foldl(map(salias)(({"1": {"1": {"1": nl}}, "0": name}) => $co(name)(nl)))(stypes)((names) => ({"1": nl, "0": name}) => cons($co(name)(nl))(names))))((names) => $gt$gt$eq($lt_(tenv$qu))(({"3": aliases, "2": types}) => $gt$gt$eq($lt_(set$slmerge(set$slfrom_list(map$slkeys(types)))(set$slmerge(set$slfrom_list(map(names)(fst)))(set$slfrom_list(map$slkeys(aliases))))))((bound) => $gt$gt$eq($lt_(map$slfrom_list(names)))((mutual_rec) => $gt$gt$eq(foldl_$gt(tenv$slnil)(salias)((tenv) => ({"1": {"1": {"1": nl, "0": body}, "0": args}, "0": name}) => (($target) => {
if ($target.type === "nil") {
return $gt$gt$eq(record_def_$gt(nl))((_21417) => $gt$gt$eq(record_local_tcon_usages(map$slfrom_list(args))(tenv)(body))((_21417) => $gt$gt$eq(record_usages_in_type(tenv$qu)(map$slmerge(map$slfrom_list(args))(mutual_rec))(body))((_21417) => $lt_(tenv$sladd_alias(tenv)(name)($co(map(args)(fst))($co(body)(nl)))))))
} ;
{
let unbound = $target;
...
let check_exhaustiveness = (tenv) => (target_type) => (patterns) => (l) => $gt$gt$eq(type$slapply_$gt(target_type))((target_type) => $gt$gt$eq($lt_(map(patterns)((pat) => cons(pattern_to_ex_pattern(tenv)($co(pat)(target_type)))(nil))))((matrix) => (($target) => {
if ($target === true) {
return $lt_($unit)
} ;
...
let templates = $target[1];
let l = $target[2];
return $gt$gt$eq($lt_(tcon("string")(l)))((string_type) => $gt$gt$eq(do_$gt(({"1": {"1": sl, "0": suffix}, "0": expr}) => $gt$gt$eq(t_expr(tenv)(expr))((t) => unify(t)(string_type)(l)))(templates))((_6800) => $lt_(string_type)))
} ;
if ($target.type === "elambda") {
let pats = $target[0];
let body = $target[1];
...
let infer_top = (tenv$qu) => (top) => (($target) => {
if ($target.type === "tdef") {
let name = $target[0];
let nl = $target[1];
let expr = $target[2];
...
let body = $target[3];
let l = $target[4];
return $gt$gt$eq(record_def_$gt(nl))((_21358) => $gt$gt$eq(record_usages_in_type(tenv$qu)(map$slnil)(body))((_21358) => $lt_(tenv(map$slnil)(map$slnil)(map$slnil)(map$slset(map$slnil)(name)($co(map(args)(({"0": name}) => name))($co(body)(nl)))))))
} ;
if ($target.type === "texpr") {
let expr = $target[0];
let l = $target[1];
...
let {"1": result, "0": {"1": {"1": subst, "0": {"1": usages, "0": types}}}} = state_f(infer(tenv)(expr))(state$slnil);
{
let type_map = foldr(map$slnil)(types)((map) => ({"1": {"1": keep, "0": type}, "0": loc}) => map$sladd(map)(loc)((($target) => {
if ($target === true) {
return type
} ;
...
let {"1": result, "0": {"1": {"1": subst, "0": {"1": usage_record, "0": types}}}} = state_f(infer_stmtss(tenv)(stmts))(state$slnil);
return $co(result)($co(applied_types(types)(subst))(usage_record))
}
let run$slusages = (tenv) => (tops) => {
let {"0": {"1": {"0": {"1": {"1": uses, "0": defns}}}}} = state_f(foldl_$gt(tenv)(tops)((tenv) => (stmt) => $gt$gt$eq(infer_stmtss(tenv)(cons(stmt)(nil)))(({"0": nenv}) => $lt_(tenv$slmerge(tenv)(nenv)))))(state$slnil);
{
let idents = map$slfrom_list(map(bag$slto_list(many(map(tops)(top$slidents))))(rev_pair));
...
let {"1": result, "0": {"1": {"1": subst, "0": {"1": usage_record, "0": types}}}} = state_f(infer(tenv)(expr))(state$slnil);
return $co(result)($co(applied_types(types)(subst))(usage_record))
