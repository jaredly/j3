...
const compile = ast => _meta => `evaluate(${JSON.stringify(ast)}, $env)`

const compile_stmt = ast => _meta => `${ast.type === 'sdef' ? `const ${ast[0]} = ` : ast.type === 'sdeftype' ? `const {${
...
const prelude = makePrelude({evaluate,evaluateStmt,unwrapArray})
