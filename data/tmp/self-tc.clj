...
(defn compile-top [ctx top]
    (match top
        (texpr expr _)              (compile ctx expr)
        (tdef name _ body _)        "const ${(sanitize name)} = ${(compile ctx body)};\n"
...
(defn with-parens [ctx expr]
    (maybe-parens (compile ctx expr) (needs-parens expr)))
    ...
    (match expr
        ...
        (estr first tpls _)     (match tpls
                                    ...
                                    _  (let [
                                           tpls (map
                                                    tpls
                                                        (fn [(, expr suffix _)]
                                                        "${${(compile ctx expr)}}${(fix-slashes suffix)}"))
                                           ]
        ...
        (elet bindings body _)  (foldr
                                    (compile ctx body)
                                        bindings
                                        (fn [body (, pat init)]
                                        "((${(pat-as-arg pat)}) => ${body})(${(compile ctx init)})"
                                            ))
        (** Curried application. (a b c) -> a(b)(c) **)
        (eapp f args _)         (foldl
                                    (with-parens ctx f)
                                        args
                                        (fn [target arg] "${target}(${(compile ctx arg)})"))
