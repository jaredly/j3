...
let type_error_$gts = ({"1": names, "0": message}) => `${message}${join("")(map(names)(({"1": loc, "0": name}) => `\n - ${name} (${its(loc)})`))}`
...
let args_1 = args;
{
let {"1": free, "0": args} = tts_list(args_1)(free_0)(locs);
{
let free_0 = free;
{
let {"1": free, "0": two} = tts_inner(r)(free_0)(locs);
...
let args_0 = args;
{
let args = cons(b)(args_0);
{
let args_0 = args;
{
let args = rev(args_0)(nil);
{
let free_0 = free;
{
let args_1 = args;
...
let free_0 = free;
{
let {"1": free, "0": one} = tts_inner(target)(free_0)(locs);
...
let free_0 = free;
{
let {"1": free, "0": arg} = tts_inner(a)(free_0)(locs);
...
let names_0 = names;
{
let names = set$slfrom_list(names_0);
...
let tenv$slset_constructors = ({"3": alias, "2": names, "1": cons, "0": types}) => (name) => (vbls) => (ncons) => tenv(types)(map$slmerge(cons)(ncons))(map$slset(names)(name)($co(vbls)(set$slfrom_list(map$slkeys(ncons)))))(alias)
...
let subst_aliases = (alias) => (type) => {
let {"1": args, "0": base} = extract_type_call(type)(nil);
{
let args_0 = args;
{
let args = map(args_0)(({"1": l, "0": arg}) => $co(subst_aliases(alias)(arg))(l));
{
let $target = base;
if ($target.type === "tcon") {
let name = $target[0];
...
if ($target === true) {
return fatal(`Wrong number of args given to alias ${name}: expected ${its(len(names))}, given ${its(len(args))}.`)
} ;
{
let subst_0 = subst;
{
...
return replace_in_type(map$slfrom_list(zip(names)(map(args)(fst))))(subst_0);
throw new Error('match fail 14042:' + JSON.stringify($target))
})($eq(len(names))(0));
return subst_aliases(alias)(subst)
}
}
};
...
return foldl(base)(args)((target) => ({"1": l, "0": arg}) => tapp(target)(arg)(l));
...
let args = $target[2];
let body = $target[3];
return split_stmts(rest)(sdefs)(stypes)(cons($co$co(name)(args)(body))(salias))(sexps)
} ;
if ($target.type === "sexpr") {
let expr = $target[0];
...
let tenv$sladd_alias = ({"3": aliases, "2": c, "1": b, "0": a}) => (name) => ({"1": body, "0": args}) => tenv(a)(b)(c)(map$slset(aliases)(name)($co(args)(body)))
let tenv$sladd_builtin_type = ({"3": d, "2": names, "1": b, "0": a}) => ({"1": args, "0": name}) => tenv(a)(b)(map$slset(names)(name)($co(args)(set$slnil)))(d)
...
let maps_0 = maps;
{
let {"1": maps, "0": text} = tts_inner(t)(maps_0)(false);
...
let bound_0 = bound;
{
let {"1": bound, "0": bag} = foldl($co(empty)(bound_0))(bindings)(({"1": bound, "0": bag}) => ({"1": init, "0": pat}) => $co(bag$sland(bag)(bag$sland(pat_externals(pat))(externals(bound)(init))))(set$slmerge(bound)(pat_names(pat))));
...
let infer_deftype = (tenv$qu) => (bound) => (tname) => (tnl) => (targs) => (constructors) => (l) => $gt$gt$eq($lt_(map(constructors)(({"0": name}) => name)))((names) => $gt$gt$eq($lt_(foldl(tcon(tname)(tnl))(targs)((body) => ({"1": al, "0": arg}) => tapp(body)(tvar(arg)(al))(l))))((final) => $gt$gt$eq($lt_(foldl(set$slnil)(targs)((free) => ({"0": arg}) => set$sladd(free)(arg))))((free_set) => $gt$gt$eq(foldl_$gt($co(map$slnil)(map$slnil))(constructors)(({"1": cons, "0": values}) => ({"3": l, "2": args, "1": nl, "0": name}) => $gt$gt$eq($lt_(map(args)((arg) => type_with_free(arg)(free_set))))((args) => $gt$gt$eq($lt_(map(args)(subst_aliases(tenv$slalias(tenv$qu)))))((args) => $gt$gt$eq(map_$gt((arg) => (($target) => {
if ($target.type === "nil") {
return $lt_(true)
} ;
{
...
let infer_stypes = (tenv$qu) => (stypes) => (salias) => $gt$gt$eq($lt_(foldl(map(salias)(({"0": name}) => name))(stypes)((names) => ({"0": name}) => cons(name)(names))))((names) => $gt$gt$eq($lt_(tenv$qu))(({"3": aliases, "2": types}) => $gt$gt$eq($lt_(set$slmerge(set$slfrom_list(map$slkeys(types)))(set$slmerge(set$slfrom_list(names))(set$slfrom_list(map$slkeys(aliases))))))((bound) => $gt$gt$eq(foldl_$gt(tenv$slnil)(salias)((tenv) => ({"2": body, "1": args, "0": name}) => (($target) => {
if ($target.type === "nil") {
return $lt_(tenv$sladd_alias(tenv)(name)($co(map(args)(fst))(body)))
} ;
{
let names = $target;
return $lt_err(type_error("Unbound types")(map(names)(({"2": l, "0": name}) => $co(name)(l))))
};
throw new Error('match fail 14529:' + JSON.stringify($target))
})(bag$slto_list(externals_type(set$slmerge(bound)(set$slfrom_list(map(args)(fst))))(body)))))((tenv) => $gt$gt$eq($lt_(tenv$slmerge(tenv)(tenv$qu)))((merged) => $gt$gt$eq(foldl_$gt(tenv)(stypes)((tenv) => ({"4": l, "3": constructors, "2": args, "1": tnl, "0": name}) => $gt$gt$eq(infer_deftype(merged)(bound)(name)(tnl)(args)(constructors)(l))((tenv$qu) => $lt_(tenv$slmerge(tenv$qu)(tenv)))))((tenv) => $gt$gt$eq($lt_(tenv$slmerge(tenv)(tenv$qu)))((tenv$qu) => $lt_(tenv))))))))
...
return $lt_err(type_error("Incompatible types")(cons($co(type_to_string(t1))(type_loc(t1)))(cons($co(type_to_string(t2))(type_loc(t2)))(nil))));
throw new Error('match fail 1317:' + JSON.stringify($target))
})($co(t1)(t2))
let unify = (t1) => (t2) => (nidx) => (l) => unify_inner(t1)(t2)(l)
...
let cases = $target[1];
let l = $target[2];
return $gt$gt$eq(new_type_var("match-res")(l))((result_var) => $gt$gt$eq(t_expr(tenv)(target))((target_type) => $gt$gt$eq(foldl_$gt($co(target_type)(result_var))(cases)(({"1": result, "0": target_type}) => ({"1": body, "0": pat}) => $gt$gt$eq(pat_and_body(tenv)(pat)(body)(target_type)(false))((body) => $gt$gt$eq($lt_subst)((subst) => $gt$gt$eq(unify_inner(type_apply(subst)(result))(body)(l))((_10271) => $gt$gt$eq($lt_subst)((subst) => $lt_($co(type_apply(subst)(target_type))(type_apply(subst)(result)))))))))(({"1": result_type}) => $lt_(result_type))))
} ;
...
let pat_and_body = (tenv) => (pat) => (body) => (value_type) => (monomorphic) => $gt$gt$eq(t_pat(tenv)(pat))(({"1": bindings, "0": pat_type}) => $gt$gt$eq(unify_inner(value_type)(pat_type)(pat_loc(pat)))((_4289) => $gt$gt$eq($lt_subst)((composed) => $gt$gt$eq($lt_(map$slmap(({"1": l, "0": t}) => $co(type_apply(composed)(t))(l))(bindings)))((bindings) => $gt$gt$eq($lt_(map$slmap(({"1": l, "0": t}) => $co((($target) => {
if ($target === true) {
...
let body = $target[3];
let l = $target[4];
return $lt_(tenv(map$slnil)(map$slnil)(map$slnil)(map$slset(map$slnil)(name)($co(map(args)(({"0": name}) => name))(body))))
} ;
if ($target.type === "sexpr") {
let expr = $target[0];
let l = $target[1];
...
let constructors = $target[3];
let l = $target[4];
return infer_deftype(tenv$qu)(set$slnil)(tname)(tnl)(targs)(constructors)(l)
} ;
...
let infer_several_inner = (bound) => (types) => ({"1": {"3": l, "2": body, "1": nl, "0": name}, "0": $var}) => $gt$gt$eq($lt_subst)((subst) => $gt$gt$eq(t_expr(tenv_apply(subst)(bound))(body))((body_type) => $gt$gt$eq($lt_subst)((subst) => $gt$gt$eq($lt_(type_apply(subst)($var)))((selfed) => $gt$gt$eq(unify_inner(selfed)(body_type)(l))((_8391) => $gt$gt$eq($lt_subst)((subst) => $gt$gt$eq($lt_(type_apply(subst)(body_type)))((body_type) => $gt$gt$eq(record_$gt(nl)(body_type)(false))((_8391) => $lt_(cons(body_type)(types))))))))))
