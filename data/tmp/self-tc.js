...
const tc_vbl = (v0) => (v1) => ({type: "tc-vbl", 0: v0, 1: v1})
const tc_con = (v0) => (v1) => (v2) => ({type: "tc-con", 0: v0, 1: v1, 2: v2})
...
const tc_exp = (type) => (cls) => (args) => foldl(`\$type_class_insts[\"${type} < ${cls}\"]`)(args)((target) => (arg) => `${target}(${arg})`);
...
const compile_tci = (info) => (($target) => {
if ($target.type === "tc-vbl") {
{
let type = $target[0];
let cls = $target[1];
return tc_exp(type)(cls)(nil)
}
}
if ($target.type === "tc-con") {
{
let type = $target[0];
let cls = $target[1];
let args = $target[2];
return tc_exp(type)(cls)(map(args)(compile_tci))
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(info);
...
{
let v = $target[0];
return foldl(sanitize(name))(v)((body) => (type_class) => `${body}(${compile_tci(type_class)})`)
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(map$slget(ctx)(loc))
