...
const go = ({0: f}) => (({1: value, 0: {0: has}}) => (($target) => {
if ($target === true) {
return right(value)
}
return left(value)
...
const $lt_right = (v) => StateT(({1: idx}) => $co($co(true)(idx))(v));

const $lt_r = (v) => $gt$gt$eq($lt_state)(({1: {1: idx}, 0: wraps}) => $gt$gt$eq(state_$gt($co(wraps)($co(true)(idx))))((_) => $lt_(v)));
...
const j$slcom = (v0) => (v1) => ({type: "j/com", 0: v0, 1: v1})
...
const simplify_block = ({0: items}) => none;
...
const con_fn = (name) => (l) => (f) => j$sllambda(cons(j$slpvar(name)(l))(nil))(right(f(j$slvar(name)(l))))(l);

const done_fn = con_fn("done");

const left_right = (either) => (name) => (l) => (usage) => (($target) => {
if ($target.type === "left") {
{
let v = $target[0];
return usage(v)
...
const cps$gt$gt$eq = ({0: f}) => (next) => StateT((state) => (({1: value, 0: {1: idx, 0: has}}) => (($target) => {
if ($target.type === "left") {
{
let v = $target[0];
...
{
let v = $target[0];
return ((name) => (({1: iner, 0: nstate}) => $co(nstate)(j$slapp(v)(cons(j$sllambda(cons(j$slpvar(name)(-1))(nil))(right(iner))(-1))(nil))(-1)))(state_f(next(j$slvar(name)(-1)))($co(true)($pl(idx)(1)))))(`v${int_to_string(idx)}`)
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(value))(f(state)));

const cps$slj2 = (trace) => (expr) => (($gt$gt$eq) => (($target) => {
if ($target.type === "evar") {
{
let n = $target[0];
let l = $target[1];
...
const compile_prim$slj = (prim) => (l) => (($target) => {
if ($target.type === "pint") {
{
let int = $target[0];
let pl = $target[1];
...
{
let pats = $target[0];
return $lt_lr(left(loop((($target) => {
if ($target.type === "nil") {
return cons(pvar("_")(l))(nil)
}
return pats
throw new Error('Failed to match. ' + valueToString($target));
})(pats))((pats) => (recur) => (($target) => {
if ($target.type === "nil") {
return fatal("gotta have pats")
}
if ($target.type === "cons" &&
$target[1].type === "nil") {
{
let final = $target[0];
return j$sllambda(cons(opt_or(pat_$gtj$slpat(final))(j$slpvar("_")(l)))(cons(j$slpvar("_ignored_effects")(l))(cons(j$slpvar("_ignored_done")(l))(nil))))(left(j$slblock(cons(j$slsexpr((($target) => {
if ($target.type === "left") {
{
let body = $target[0];
...
let arg = $target[0];
let rest = $target[1];
return j$sllambda(cons(opt_or(pat_$gtj$slpat(arg))(j$slpvar("_")(l)))(cons(j$slpvar("_ignored_effects")(l))(cons(j$slpvar("\$done")(l))(nil))))(right(j$slapp(j$slvar("\$done")(l))(cons(recur(rest))(nil))(l)))(l)
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(pats))))
...
{
let v = $target[0];
return $gt$gt$eq($lt_state)(({1: {1: idx, 0: flag}, 0: wraps}) => $gt$gt$eq($lt_(`${n}_${int_to_string(idx)}`))((name) => $gt$gt$eq(state_$gt($co(cons($co(v)(name))(wraps))($co(flag)($pl(1)(idx)))))((_) => $lt_(j$slvar(name)(-1)))))
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(v);
...
let arg = $target[0];
let rest = $target[1];
return (({1: assign, 0: check}) => (({1: a2, 0: c2}) => $co(concat(cons(check)(cons(c2)(nil))))(concat(cons(assign)(cons(a2)(nil)))))(pat_loop_list(target)(rest)($pl(i)(1))))(compile_pat_list(arg)(j$slindex(target)(j$slprim(j$slint(i)(-1))(-1))(-1)))
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(args);
...
let name = $target[0];
let l = $target[1];
return $co(nil)(cons(j$sllet(j$slpvar(sanitize(name))(l))(j$slcom("pat-list")(target))(l))(nil))
}
}
if ($target.type === "pcon") {
{
...
let args = $target[2];
let l = $target[3];
return (({1: assign, 0: check}) => $co(cons(j$slbin("===")(j$slattr(target)("type")(l))(j$slstr(name)(nil)(l))(l))(check))(assign))(pat_loop_list(target)(args)(0))
}
}
if ($target.type === "penum" &&
$target[2].type === "none") {
...
let arg = $target[2][0];
let l = $target[3];
return (({1: assign, 0: check}) => $co(cons(j$slbin("===")(j$slattr(target)("tag")(l))(j$slstr(name)(nil)(l))(l))(check))(assign))(compile_pat_list(arg)(j$slattr(target)("arg")(l)))
}
}
if ($target.type === "precord") {
{
...
let spread = $target[1];
let l = $target[2];
return (({1: assign, 0: check}) => (($target) => {
if ($target.type === "none") {
...
{
let spread = $target[0];
return (({1: a, 0: c}) => $co(concat(cons(check)(cons(c)(nil))))(concat(cons(assign)(cons(a)(nil)))))(compile_pat_list(spread)(target))
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(spread))(loop($co(items)($co(nil)(nil)))(({1: {1: assign, 0: check}, 0: items}) => (recur) => (($target) => {
if ($target.type === "nil") {
return $co(check)(assign)
}
if ($target.type === "cons" &&
...
let pat = $target[0][1];
let rest = $target[1];
return (({1: a, 0: c}) => $co(concat(cons(check)(cons(c)(nil))))(concat(cons(assign)(cons(a)(nil)))))(compile_pat_list(pat)(j$slattr(target)(name)(l)))
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(items)))
...
const cps$slj = (trace) => (expr) => ((cps) => (($target) => {
if ($target.type === "eapp" &&
$target[1].type === "cons" &&
$target[1][1].type === "nil") {
{
...
let arg = $target[1][0];
let l = $target[2];
return ((target) => ((arg) => right(done_fn(l)((done) => left_right(target)("target")(l)((target) => left_right(arg)("arg")(l)((arg) => j$slapp(target)(cons(arg)(cons(j$slvar("\$lbeffects\$rb")(l))(cons(done)(nil))))(l))))))(cps(arg)))(cps(target))
}
}
if ($target.type === "eapp" &&
$target[1].type === "cons") {
...
let body = $target[1];
let l = $target[2];
return ((pat) => left(j$sllambda(cons(pat)(cons(j$slpvar("\$lbeffects\$rb")(l))(cons(j$slpvar("\$done")(l))(nil))))(right((($target) => {
if ($target.type === "left") {
{
let body = $target[0];
return j$slapp(j$slvar("\$done")(l))(cons(body)(nil))(l)
...
throw new Error('Failed to match. ' + valueToString($target));
})(check)))(j$slblock(cons(inner)(nil)))(none)(l)
throw new Error('Failed to match. ' + valueToString($target));
})(check)))((($target) => {
if ($target.type === "nil") {
return inner
}
return j$slsblock(j$slblock(concat(cons(assign)(cons(cons(inner)(nil))(nil)))))(l)
throw new Error('Failed to match. ' + valueToString($target));
...
let body = $target[1];
let l = $target[2];
return left(j$sllambda(cons(opt_or(pat_$gtj$slpat(arg))(j$slpvar("_")(l)))(cons(j$slpvar(efvbl)(l))(cons(j$slpvar("\$done")(l))(nil))))(right((($target) => {
if ($target.type === "left") {
{
let body = $target[0];
return j$slapp(j$slvar("\$done")(l))(cons(body)(nil))(l)
...
const compile_top_cps$slj = (top) => (trace) => (($target) => {
if ($target.type === "texpr") {
{
let expr = $target[0];
let l = $target[1];
...
throw new Error('Failed to match. ' + valueToString($target));
})(type_info)))((expr) => (type_info) => (ctx) => ((expr) => j$slcompile(ctx)(provide_empty_effects(right(map$slexpr(simplify_js)(compile_cps$slj(expr)(ctx))))))((($target) => {
if ($target.type === "tvar") {
return expr
}
