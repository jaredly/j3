...
let one = $target[0];
let rest = $target[1];
return j$slapp(j$slvar("\$co")(l))(cons(one)(cons(recur(rest))(nil)))(l)
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(args)))))
...
{
let f = $target[0];
return `({\$type: 'thunk', f: (\$env, \$update) => {\nconst \$produce = [];\nconst \$lbeffects\$rb = ${builtin_effects};\n${j$slcompile_stmt(ctx)(map$slstmt(simplify_js)(j$slsexpr(f(j$slraw("((final_value) => {\n  \$produce.push(final_value);\n  \$update(\$produce, false)\n})")(l)))(l)))}\n}})`
}
}
throw new Error('Failed to match. ' + valueToString($target));
})(cps$slj3(ctx)(expr))
