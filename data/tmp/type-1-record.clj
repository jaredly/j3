...
(defn record-type [types loc type]
    (, type (bag/and (one (, loc type)) types)))

(def just-type (record-type empty))
    ...
    (match expr
        ...
        (evar name l)                       (match (tenv/type tenv name)
                                                ...
                                                (some found) (let [(,, t _ nidx) (instantiate found nidx)]
                                                                 (,, map/nil (just-type l t) nidx)))
        (equot _ l)                         (,, map/nil (just-type l (tcon "expr" l)) nidx)
        (equot/stmt _ l)                    (,, map/nil (just-type l (tcon "stmt" l)) nidx)
        (equot/pat _ l)                     (,, map/nil (just-type l (tcon "pat" l)) nidx)
        (equot/type _ l)                    (,, map/nil (just-type l (tcon "type" l)) nidx)
        (equotquot _ l)                     (,, map/nil (just-type l (tcon "cst" l)) nidx)
        (eprim prim l)                      (,, map/nil (just-type l (t-prim prim)) nidx)
        (estr first templates l)            (let [
                                                string-type            (tcon "string" l)
                                                (,, subst record nidx) (foldr
                                                                           (,, map/nil empty nidx)
                                                                               templates
                                                                               (fn [(,, subst record nidx) (,, expr suffix sl)]
                                                                               (let [
                                                                                   (,, s2 (, t record2) nidx) (t-expr tenv expr nidx)
                                                                                   (, s3 nidx)                (unify t string-type nidx)]
                                                                                   (,,
                                                                                       (compose-subst s3 (compose-subst s2 subst))
                                                                                           (bag/and record record2)
                                                                                           nidx))))]
                                                (,, subst (record-type record l string-type) nidx))
        ...
        (elambda name nl body l)            (let [
                                                ...
                                                (,, body-subst (, body-type record) nidx) (t-expr env-with-name body nidx)]
                                                (,,
                                                    body-subst
                                                        (record-type
                                                        record
        ...
        (eapp target arg l)                 (let [
                                                ...
                                                (,, target-subst (, target-type target-record) nidx) (t-expr tenv target nidx)
                                                (,, arg-subst (, arg-type arg-record) nidx)          (t-expr (tenv-apply target-subst tenv) arg nidx)
                                                ...
                                                (,,
                                                    (compose-subst
                                                        ...
                                                        (record-type
                                                        (bag/and target-record arg-record)
                                                            l
        ...
        (ematch target cases l)             (let [
                                                ...
                                                (,, target-subst (, target-type record) nidx)    (t-expr tenv target nidx)
                                                (,,, _ target-subst (, result-type record) nidx) (foldr
                                                                                                     (,,, target-type target-subst (, result-var record) nidx)
                                                                                                         cases
                                                                                                         (fn [(,,, target-type subst (, result record) nidx) (, pat body-expr)]
                                                                                                         (let [
                                                                                                             (,, subst (, body record) nidx) (pat-and-body
                                                                                                                                                 tenv
                                                                                                                                                     pat
                                                                                                                                                     body-expr
                                                                                                                                                     (,, subst (, target-type record) nidx))
                                                                                                             ...
                                                                                                             (,,,
                                                                                                                 (type-apply (compose-subst subst unified-subst) target-type)
                                                                                                                     ...
                                                                                                                     nidx))))]
                                                (,, target-subst (, result-type record) nidx))
        _                                   (fatal "cannot infer type for ${(valueToString expr)}")))

(defn pat-and-body [tenv pat body (,, value-subst (, value-type record) nidx)]
    (let [
        (,, (, pat-type record) bindings nidx)     (t-pat tenv pat nidx)
        ...
        (,, body-subst (, body-type record2) nidx) (t-expr
...
(defn t-pat [tenv pat nidx]
    (match pat
        (pany nl)             (let [(, var nidx) (new-type-var "any" nidx)]
                                  (,, (just-type nl var) map/nil nidx))
        (pvar name nl)        (let [(, var nidx) (new-type-var name nidx)]
                                  (,, (just-type nl var) (map/set map/nil name var) nidx))
        (pstr _ nl)           (,, (just-type nl (tcon "string" nl)) map/nil nidx)
        (pprim (pbool _ _) l) (,, (just-type l (tcon "bool" l)) map/nil nidx)
        (pprim (pint _ _) l)  (,, (just-type l (tcon "int" l)) map/nil nidx)
        (pcon name args l)    (let [
                                  ...
                                  _                                (if (!= (len args) (len cargs))
                                                                       (fatal
                                                                           ...
                                                                           0)
                                  (,,, subst bindings record nidx) (foldl
                                                                       (,,, map/nil map/nil empty nidx)
                                                                           zipped
                                                                           (fn [(,,, subst bindings record nidx) (, arg carg)]
                                                                           (let [
                                                                               (,, (, pat-type record2) pat-bind nidx) (t-pat tenv arg nidx)
                                                                               ...
                                                                               (,,,
                                                                                   (compose-subst unified-subst subst)
                                                                                       ...
                                                                                       nidx))))]
                                  (,,
                                      (record-type record l (type-apply subst tres))
                                          (map/map (type-apply subst) bindings)
                                          nidx))))

(defn infer [tenv expr]
    (let [(,, s (, t record) nidx) (t-expr tenv expr 0)] (type-apply s t)))
...
(defn infer-stmt [tenv' stmt]
    (match stmt
        (sdef name nl expr l)                     (let [
                                                      ...
                                                      (,, s (, t record) nidx) (t-expr self-bound expr nidx)
    ...
    (let [
        ...
        (,, subst types nidx) (foldr
                                  (,, map/nil [] nidx)
                                      ...
                                      (let [
                                          (,, body-subst (, body-type record) nidx) (t-expr ;(bound) (tenv-apply subst bound) body nidx)
